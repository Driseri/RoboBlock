...
Blockly.Arduino['ESPBOY-1']=function(block){
    var valuetext = block.getFieldValue('text');
    Blockly.Arduino.definitions_['definitions_ESPBOY-1'] = ' \n'+
'#include <Arduino.h> \n'+
...
'Adafruit_NeoPixel ledpix(1, LEDPIN, NEO_GRB + NEO_KHZ800); \n'+
' \n'+
...
'  0x00, 0xE0, 0xF7, 0x33, 0xF0, 0xF1, 0xF8, 0x00, 0x00, 0x00, 0x00, 0x00,  \n'+
  ...
  ' ';
    ...
    Blockly.Arduino.setups_['setups_ESPBOY-1']='  \n'+
  '    \n'+
' Serial.begin(115200);    \n'+
...
'   //tft.fillScreen(ST77XX_BLACK);   \n'+
' ';
    var code = ' uint8_t keypressed;   \n  static uint8_t rnd = 0;   \n   keypressed=ESPboyGetKeys();  \n';
    return code
};

Blockly.Arduino['ESPBOY-actuator_tonedure'] = function(block) {
 var dropdown_pin = '0';
...
Blockly.Arduino['espboy_button']=function(block){
    var value_dropdown = block.getFieldValue('_dropdown');
   if(value_dropdown == 'keypressed'){
    var code = 'keypressed';
 }else{
     var code = 'keypressed&'+value_dropdown+'';
 }
    return [code, Blockly.Arduino.ORDER_ATOMIC];
};

Blockly.Arduino['espboy_rgb']=function(block){
    var valuecolour = block.getFieldValue('colour');
var red = parseInt(valuecolour.substring(1,3),16);
var green = parseInt(valuecolour.substring(3,5),16);
var blue = parseInt(valuecolour.substring(5,7),16);
    var code = 'ledpix.setPixelColor(0, ledpix.Color('+red+', '+green+', '+blue+'));  \n  ledpix.show(); \n ';
    return code
};

 Blockly.Arduino['ESPBOY-lcd_128x128px_spi_text'] = function(block) {
...
var code = '     tft.setCursor('+X_Nokia+', '+Y_Nokia+');  \n   tft.setTextColor('+color+',0);  \n    tft.setTextSize('+size+');  \n   tft.println('+texttoprint+');  \n  ';

return code;
};

 Blockly.Arduino['ESPBOY-lcd_128x128px_spi_pix'] = function(block) {

var X_Nokia = Blockly.Arduino.valueToCode(block, 'X_Nokia', Blockly.Arduino.ORDER_ATOMIC);
var Y_Nokia = Blockly.Arduino.valueToCode(block, 'Y_Nokia', Blockly.Arduino.ORDER_ATOMIC);
var color = this.getFieldValue('color'); 

var code = ' tft.drawPixel('+X_Nokia+', '+Y_Nokia+','+color+'); \n  ';

return code;
};

 Blockly.Arduino['ESPBOY-lcd_128x128px_spi_line'] = function(block) {
...
var code = 'tft.drawLine('+X_Nokia+', '+Y_Nokia+','+X_Nokia1+', '+Y_Nokia1+','+color+');  \n  ';

return code;
};

 Blockly.Arduino['ESPBOY-lcd_128x128px_spi_box'] = function(block) {
...
var logic = this.getFieldValue('LOGIC');
  ...
  if(logic == 'FALSE'){
  var code = ' tft.drawRoundRect('+X_Nokia+', '+Y_Nokia+', '+Y_Nokia2+', '+X_Nokia2+', '+Y_Nokia3+', '+color+');   \n  ';
  }else{
  var code = ' tft.fillRoundRect('+X_Nokia+', '+Y_Nokia+', '+Y_Nokia2+', '+X_Nokia2+', '+Y_Nokia3+', '+color+');   \n  ';

  }
return code;
};

Blockly.Arduino['ESPBOY-st7735_backgroundcolor'] = function(block) {
...
Blockly.Arduino["base_code_comment"]=function(block){
...
Blockly.Arduino["tempo_sans_delay"]=function(block){
  ...
  var temps="temps"+delay_time+_u;
  Blockly.Arduino.definitions_["temporisation"+delay_time+_u]="long "+temps+"=0 ;";
    switch (_u) {
        case "u":
...
Blockly.Arduino['LedStrip_WS2812B_init_top2'] = function(block) {
  
  var numberstrip = Blockly.Arduino.valueToCode(block, "PIN", Blockly.Arduino.ORDER_ATOMIC); //"1";  
  var pin_ledrgb = Blockly.Arduino.valueToCode(block, "PIN", Blockly.Arduino.ORDER_ATOMIC);  
  var numpixels = Blockly.Arduino.valueToCode(block, "PINR", Blockly.Arduino.ORDER_ATOMIC);
  var Pixel_rgb = Blockly.Arduino.valueToCode(block, "Pixel_rgb", Blockly.Arduino.ORDER_ATOMIC);
...
Blockly.Arduino.definitions_['define_ledRGB_WS2812B_top' + numberstrip] = ' Adafruit_NeoPixel strip'+numberstrip+'('+numpixels+', '+pin_ledrgb+', NEO_GRB + NEO_KHZ800); \n  ';
 ...
 Blockly.Arduino.setups_['setup_ledRGB_WS2812B_top'+numberstrip] = '  strip'+numberstrip+'.begin();  \n  strip'+numberstrip+'.show();  \n  strip'+numberstrip+'.setBrightness(50);  \n';
  ...
  var code = ' Pixcil = 0; \n NeoPix = '+numpixels+'; \n  for (int count = 0; count < NeoPix; count++) { \n    Pixcil = Pixcil + 1; \n   strip'+numberstrip+'.setBrightness('+Pixel_rgb+'); \n     strip'+numberstrip+'.setPixelColor(Pixcil-1, strip'+numberstrip+'.Color('+red+','+green+','+blue+')); \n          } \n   Pixcil = 0; \n  strip'+numberstrip+'.show();  \n ';
 
  return code;
};

Blockly.Arduino['LedStrip_WS2812B_init_top8x8'] = function(block) {
    var valuepin = block.getFieldValue('pin');

    // Собираем все значения цветов в массив
    var values = [];
    for (var i = 1; i <= 64; i++) {
        var value = block.getFieldValue('x' + i);
        values.push(value);
    }

    // Определения
    Blockly.Arduino.definitions_['LedStrip_WS2812B_init_top'] = '#include <Adafruit_NeoPixel.h>\n#ifdef __AVR__\n  #include <avr/power.h>\n#endif\n';
    Blockly.Arduino.definitions_['LedStrip_WS2812B_init_top8x8' + valuepin] = 'Adafruit_NeoPixel pixels_' + valuepin + ' = Adafruit_NeoPixel(64, ' + valuepin + ', NEO_GRB + NEO_KHZ800);\n';
    Blockly.Arduino.setups_['LedStrip_WS2812B_init_top8x8' + valuepin] = 'pixels_' + valuepin + '.begin();\n';

    // Преобразуем цвета и записываем их
    var code = 'pixels_' + valuepin + '.setBrightness(50);\n';
    for (var i = 0; i < 64; i++) {
        var red = parseInt(values[i].substring(1, 3), 16);
        var green = parseInt(values[i].substring(3, 5), 16);
        var blue = parseInt(values[i].substring(5, 7), 16);
        code += 'pixels_' + valuepin + '.setPixelColor(' + i + ', pixels_' + valuepin + '.Color(' + red + ',' + green + ',' + blue + '));\n';
    }

    code += 'pixels_' + valuepin + '.show();\n';
    return code;
};

Blockly.Arduino['LedStrip_WS2812B_init_top16x16'] = function(block) {
    var valuepin = block.getFieldValue('pin');

    // Собираем все значения цветов в массив
    var values = [];
    for (var i = 1; i <= 256; i++) {  // Обрабатываем 256 пикселей
        var value = block.getFieldValue('x' + i);
        values.push(value);
    }
    ...
    // Определения
    Blockly.Arduino.definitions_['LedStrip_WS2812B_init_top'] = '#include <Adafruit_NeoPixel.h>\n#ifdef __AVR__\n  #include <avr/power.h>\n#endif\n';
    Blockly.Arduino.definitions_['LedStrip_WS2812B_init_top16x16' + valuepin] = 'Adafruit_NeoPixel pixels_' + valuepin + ' = Adafruit_NeoPixel(256, ' + valuepin + ', NEO_GRB + NEO_KHZ800);\n';
    Blockly.Arduino.setups_['LedStrip_WS2812B_init_top16x16' + valuepin] = 'pixels_' + valuepin + '.begin();\n';

    // Преобразуем цвета и записываем их
    var code = 'pixels_' + valuepin + '.setBrightness(50);\n';
    for (var i = 0; i < 256; i++) {  // Преобразуем цвета для каждого из 256 пикселей
        var red = parseInt(values[i].substring(1, 3), 16);
        var green = parseInt(values[i].substring(3, 5), 16);
        var blue = parseInt(values[i].substring(5, 7), 16);
        code += 'pixels_' + valuepin + '.setPixelColor(' + i + ', pixels_' + valuepin + '.Color(' + red + ',' + green + ',' + blue + '));\n';
...
Blockly.Arduino['DisplayTM1637_set_completenumber6'] = function(block) {
  
 var value = Blockly.Arduino.valueToCode(block, 'NNN', Blockly.Arduino.ORDER_ATOMIC); 
 
  var code = ' tm1637.point(POINT_OFF); \n   if ('+value+' >= 100000) {tm1637.display(2,('+value+'/long(pow(10, 5)))%10);} else { tm1637.display(2,17);} \n   if ('+value+' >= 10000) {tm1637.display(1,('+value+'/long(pow(10, 4)))%10);} else { tm1637.display(1,17);} \n  if ('+value+' >= 1000) {tm1637.display(0,('+value+'/long(pow(10, 3)))%10);} else { tm1637.display(0,17);} \n    if ('+value+' >= 100) {tm1637.display(5,('+value+'/long(pow(10, 2)))%10);} else { tm1637.display(5,17);} \n   if ('+value+' >= 10) {tm1637.display(4,('+value+'/long(pow(10, 1)))%10);} else { tm1637.display(4,17);} \n   tm1637.display(3,('+value+'/long(pow(10, 0)))%10); \n ';

 return code;
};

Blockly.Arduino['DisplayTM1637_set_completenumber61'] = function(block) {
  
 var value = Blockly.Arduino.valueToCode(block, 'NNN', Blockly.Arduino.ORDER_ATOMIC); 
 
  var code = ' tm1637.point(POINT_OFF); \n     if ('+value+' >= 100) {tm1637.display(2,('+value+'/long(pow(10, 2)))%10);} else { tm1637.display(2,17);} \n   if ('+value+' >= 10) {tm1637.display(1,('+value+'/long(pow(10, 1)))%10);} else { tm1637.display(1,17);} \n   tm1637.display(0,('+value+'/long(pow(10, 0)))%10); \n ';

 return code;
};

Blockly.Arduino['DisplayTM1637_set_completenumber62'] = function(block) {
...
return code;
 ...
 Blockly.Arduino['lcdtime'] = function(block) {

  var value_column = Blockly.Arduino.valueToCode(block, 'column', Blockly.Arduino.ORDER_ATOMIC);
  var value_row = Blockly.Arduino.valueToCode(block, 'row', Blockly.Arduino.ORDER_ATOMIC);

 Blockly.Arduino.definitions_['lcdtime'] = '      \n  '+ 
   ' int xtime = 0;  \n  '+ 
   ' int ytime = 0;  \n  '+ 
   ' int ntime = 0;  \n  '+ 
   ' int mtime = 0;  \n  '+ 
   ...
   ' void x0() {  \n  '+ 
   ' lcd.createChar(1, customChar1);lcd.setCursor(xtime, 0);lcd.write(1);lcd.createChar(2, customChar2);lcd.setCursor((xtime + 1), 0);lcd.write(2);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 0);lcd.write(1);lcd.createChar(1, customChar1);lcd.setCursor(xtime, 1);lcd.write(1);lcd.createChar(3, customChar3);lcd.setCursor((xtime + 1), 1);lcd.write(3);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 1);lcd.write(1);  \n  '+ 
   ' }  \n  '+ 
   ' void x5() {  \n  '+ 
   ' lcd.createChar(1, customChar1);lcd.setCursor(xtime, 0);lcd.write(1);lcd.createChar(5, customChar5);lcd.setCursor((xtime + 1), 0);lcd.write(5);lcd.createChar(5, customChar5);lcd.setCursor((xtime + 2), 0);lcd.write(5);lcd.createChar(3, customChar3);lcd.setCursor(xtime, 1);lcd.write(3);lcd.createChar(3, customChar3);lcd.setCursor((xtime + 1), 1);lcd.write(3);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 1);lcd.write(1);  \n  '+ 
   ' }  \n  '+ 
   ' void n() {  \n  '+ 
   '   if (ytime == 0) {x0();}if (ytime == 1) {x1();}if (ytime == 2) {x2();}if (ytime == 3) {x3();}if (ytime == 4) {x4();}if (ytime == 5) {x5();}if (ytime == 6) {x6();}if (ytime == 7) {x7();}if (ytime == 8) {x8();}if (ytime == 9) {x9();}if (ytime == 10) {x10();}  \n  '+ 
   ' }  \n  '+ 
   ' void NB2() {  \n  '+ 
   ' if (mtime >= 10 & mtime < 100) {xtime = 9;r3time = mtime / 10;ytime = r3time;n();xtime = 13;r4time = mtime - r3time * 10;ytime = r4time;n();} if (mtime >= 0 & mtime < 10) {xtime = 9;ytime = 0;n();xtime = 13;r4time = mtime;ytime = r4time;n();}  \n  '+ 
   ' }  \n  '+ 
   ' void NB1() {  \n  '+ 
   ' if (ntime >= 10 & ntime < 100) {xtime = 0;r3time = ntime / 10;ytime = r3time;n();xtime = 4;r4time = ntime - r3time * 10;ytime = r4time;n();} if (ntime >= 0 & ntime < 10) {xtime = 0;ytime = 10;n();xtime = 4;r4time = ntime;ytime = r4time;n();}  \n  '+ 
   ' }  \n  '+ 
   ' void x1() {  \n  '+ 
   ' lcd.createChar(2, customChar2);lcd.setCursor(xtime, 0);lcd.write(2);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 1), 0);lcd.write(1);lcd.createChar(4, customChar4);lcd.setCursor((xtime + 2), 0);lcd.write(4);lcd.createChar(3, customChar3);lcd.setCursor(xtime, 1);lcd.write(3);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 1), 1);lcd.write(1);lcd.createChar(3, customChar3);lcd.setCursor((xtime + 2), 1);lcd.write(3);  \n  '+ 
   '    }  \n  '+ 
   ' void x6() {  \n  '+ 
   ' lcd.createChar(1, customChar1);lcd.setCursor(xtime, 0);lcd.write(1);lcd.createChar(5, customChar5);lcd.setCursor((xtime + 1), 0);lcd.write(5);lcd.createChar(5, customChar5);lcd.setCursor((xtime + 2), 0);lcd.write(5);lcd.createChar(1, customChar1);lcd.setCursor(xtime, 1);lcd.write(1);lcd.createChar(3, customChar3);lcd.setCursor((xtime + 1), 1);lcd.write(3);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 1);lcd.write(1);  \n  '+ 
   '    }  \n  '+ 
   ' void x2() {  \n  '+ 
   ' lcd.createChar(2, customChar2);lcd.setCursor(xtime, 0);lcd.write(2);lcd.createChar(2, customChar2);lcd.setCursor((xtime + 1), 0);lcd.write(2);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 0);lcd.write(1);lcd.createChar(1, customChar1);lcd.setCursor(xtime, 1);lcd.write(1);lcd.createChar(5, customChar5);lcd.setCursor((xtime + 1), 1);lcd.write(5);lcd.createChar(3, customChar3);lcd.setCursor((xtime + 2), 1);lcd.write(3);  \n  '+ 
   '    }  \n  '+ 
   ' void x7() {  \n  '+ 
   ' lcd.createChar(2, customChar2);lcd.setCursor(xtime, 0);lcd.write(2);lcd.createChar(2, customChar2);lcd.setCursor((xtime + 1), 0);lcd.write(2);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 0);lcd.write(1);lcd.createChar(4, customChar4);lcd.setCursor(xtime, 1);lcd.write(4);lcd.createChar(4, customChar4);lcd.setCursor((xtime + 1), 1);lcd.write(4);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 1);lcd.write(1);  \n  '+ 
   '    }  \n  '+ 
   ' void x3() {  \n  '+ 
   ' lcd.createChar(2, customChar2);lcd.setCursor(xtime, 0);lcd.write(2);lcd.createChar(5, customChar5);lcd.setCursor((xtime + 1), 0);lcd.write(5);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 0);lcd.write(1);lcd.createChar(3, customChar3);lcd.setCursor(xtime, 1);lcd.write(3);lcd.createChar(3, customChar3);lcd.setCursor((xtime + 1), 1);lcd.write(3);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 1);lcd.write(1);  \n  '+ 
   '    }  \n  '+ 
   ' void x8() {  \n  '+ 
   ' lcd.createChar(1, customChar1);lcd.setCursor(xtime, 0);lcd.write(1);lcd.createChar(5, customChar5);lcd.setCursor((xtime + 1), 0);lcd.write(5);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 0);lcd.write(1);lcd.createChar(1, customChar1);lcd.setCursor(xtime, 1);lcd.write(1);lcd.createChar(3, customChar3);lcd.setCursor((xtime + 1), 1);lcd.write(3);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 1);lcd.write(1);  \n  '+ 
   '    }  \n  '+ 
   ' void x4() {  \n  '+ 
   ' lcd.createChar(1, customChar1);lcd.setCursor(xtime, 0);lcd.write(1);lcd.createChar(4, customChar4);lcd.setCursor((xtime + 1), 0);lcd.write(4);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 0);lcd.write(1);lcd.createChar(2, customChar2);lcd.setCursor(xtime, 1);lcd.write(2);lcd.createChar(2, customChar2);lcd.setCursor((xtime + 1), 1);lcd.write(2);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 1);lcd.write(1);  \n  '+ 
   '    }  \n  '+ 
   ' void x9() {  \n  '+ 
   ' lcd.createChar(1, customChar1);lcd.setCursor(xtime, 0);lcd.write(1);lcd.createChar(5, customChar5);lcd.setCursor((xtime + 1), 0);lcd.write(5);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 0);lcd.write(1);lcd.createChar(3, customChar3);lcd.setCursor(xtime, 1);lcd.write(3);lcd.createChar(3, customChar3);lcd.setCursor((xtime + 1), 1);lcd.write(3);lcd.createChar(1, customChar1);lcd.setCursor((xtime + 2), 1);lcd.write(1);  \n  '+ 
   '    }  \n  '+ 
   ' void x10() {  \n  '+ 
   ' lcd.createChar(4, customChar4);lcd.setCursor(xtime, 0);lcd.write(4);lcd.createChar(4, customChar4);lcd.setCursor((xtime + 1), 0);lcd.write(4);lcd.createChar(4, customChar4);lcd.setCursor((xtime + 2), 0);lcd.write(4);lcd.createChar(4, customChar4);lcd.setCursor(xtime, 1);lcd.write(4);lcd.createChar(4, customChar4);lcd.setCursor((xtime + 1), 1);lcd.write(4);lcd.createChar(4, customChar4);lcd.setCursor((xtime + 2), 1);lcd.write(4);  \n  '+ 
   '    }      \n';

 var code = 'NB1(); ntime = '+value_column+'; NB2(); mtime = '+value_row+ '; lcd.createChar(6, customChar6);lcd.setCursor(7, 0);lcd.write(6);lcd.createChar(7, customChar7);lcd.setCursor(8, 0);lcd.write(7);lcd.createChar(6, customChar6);lcd.setCursor(7, 1);lcd.write(6);lcd.createChar(7, customChar7);lcd.setCursor(8, 1);lcd.write(7);delay(500);lcd.createChar(4, customChar4);lcd.setCursor(7, 0);lcd.write(4);lcd.createChar(4, customChar4);lcd.setCursor(8, 0);lcd.write(4);lcd.createChar(4, customChar4);lcd.setCursor(7, 1);lcd.write(4);lcd.createChar(4, customChar4);lcd.setCursor(8, 1);lcd.write(4);delay(500); \n ';
return code;
};
...
var code = '  LCD.setFont('+point+'); \n  LCD.printNumI(long('+Nokia_number+'), '+X_Nokia+', '+Y_Nokia+', 2, '+no1+'); \n  ';
...
Blockly.Arduino['web_esp_conect']=function(block){
 ...
 '   ptr +="<meta name=\\"apple-mobile-web-app-status-bar-style\\" content=\\"black\\"> "; \n '+
 '   ptr +="<meta name=\\"apple-mobile-web-app-title\\" content=\\"'+valuetitle+'\\"> "; \n '+
 '   ptr +="<link rel=\\"apple-touch-icon\\" href=\\"https://ardublock.ru/js/favicon.ico\\"> "; \n '+
 '   ptr +="<style>.button { -webkit-user-select: none; -webkit-touch-callout: none; user-select: none;}"; \n '+
...
Blockly.Arduino['web_esp_conect_b']=function(block){
  ...
  var valuename = block.getFieldValue('name');
   ...
   Blockly.Arduino['web_esp_conect_bs']=function(block){
    var valueid = block.getFieldValue('id');
    var valuename = block.getFieldValue('name');
    var valuebstatement = Blockly.Arduino.statementToCode(block, 'bstatement');
	var valuebstatement2 = Blockly.Arduino.statementToCode(block, 'bstatement2');
    //var code = 'ptr +="<a class=\\"button button-off\\"  onclick=\\"send('+valueid+')\\">'+valuename+'</a>"; \n   if (a == '+valueid+') { \n    '+valuebstatement+' \n   } \n  ';
    if (valueid == '1'){
	var code = 'ptr +="<button style=\\"width:300px;height: 100px;\\" class=\\"button button-off\\"  ontouchstart=\\"send(1)\\" ontouchend=\\"send(2)\\">'+valuename+'</button>"; \n if (a == 1) {\n '+valuebstatement+' \n }\n if (a == 2) {\n '+valuebstatement2+' \n }\n  ';
  }
      if (valueid == '2'){
	var code = 'ptr +="<button style=\\"width:300px;height: 100px;\\" class=\\"button button-off\\"  ontouchstart=\\"send(3)\\" ontouchend=\\"send(4)\\">'+valuename+'</button>"; \n if (a == 3) {\n '+valuebstatement+' \n }\n if (a == 4) {\n '+valuebstatement2+' \n }\n  ';
  }
      if (valueid == '3'){
	var code = 'ptr +="<button style=\\"width:300px;height: 100px;\\" class=\\"button button-off\\"  ontouchstart=\\"send(5)\\" ontouchend=\\"send(6)\\">'+valuename+'</button>"; \n if (a == 5) {\n '+valuebstatement+' \n }\n if (a == 6) {\n '+valuebstatement2+' \n }\n  ';
  }
      if (valueid == '4'){
	var code = 'ptr +="<button style=\\"width:300px;height: 100px;\\" class=\\"button button-off\\"  ontouchstart=\\"send(7)\\" ontouchend=\\"send(8)\\">'+valuename+'</button>"; \n if (a == 7) {\n '+valuebstatement+' \n }\n if (a == 8) {\n '+valuebstatement2+' \n }\n  ';
  }
      if (valueid == '5'){
	var code = 'ptr +="<button style=\\"width:300px;height: 100px;\\" class=\\"button button-off\\"  ontouchstart=\\"send(9)\\" ontouchend=\\"send(10)\\">'+valuename+'</button>"; \n if (a == 9) {\n '+valuebstatement+' \n }\n if (a == 10) {\n '+valuebstatement2+' \n }\n  ';
  }
  return code
};
...
if(number == 8){
 ...
 Blockly.Arduino['ledi_ws2812rgbled'] = function(block) {
  
  var numberstrip = block.getFieldValue('pin'); // номер
  var pin_ledrgb = block.getFieldValue('pin'); // пин ленты
  var numpixels = block.getFieldValue('led'); // количество пикселей в ленте
  var rgbled = block.getFieldValue('rgbled'); 
...
if(number == 6){
  var code = ' Pixcil = '+valueledon+'; \n NeoPix = '+valueledoff+' - Pixcil; \n  for (int count = 0; count < NeoPix; count++) { \n    Pixcil = Pixcil + 1; \n   strip'+numberstrip+'.setBrightness('+rgbled+'); \n     strip'+numberstrip+'.setPixelColor(Pixcil-1, strip'+numberstrip+'.Color('+red+','+green+','+blue+')); \n         } \n   Pixcil = '+valueledon+'; \n strip'+numberstrip+'.show(); \n   delay('+pixel_number+');   \n ';
}
...
if(number == 8){
 ...
 Blockly.Arduino['ledi_ws2812rgb'] = function(block) {
...
Blockly.Arduino.definitions_['define_ledRGB_WS2812B_top' + numberstrip] = ' Adafruit_NeoPixel strip'+numberstrip+'('+numpixels+', '+pin_ledrgb+', NEO_GRB + NEO_KHZ800);   \n ';
 ...
 Blockly.Arduino.setups_['setup_ledRGB_WS2812B_top'+numberstrip] = '  strip'+numberstrip+'.begin();  \n  strip'+numberstrip+'.show();  \n  strip'+numberstrip+'.setBrightness(50);  \n';
  ...
  var valuews = block.getFieldValue('ws'); // выбор ленты модели
  if(valuews == 0){
  var red = block.getFieldValue('rr');  
  var green = block.getFieldValue('gg');
  var blue = block.getFieldValue('bb');
  }
  if(valuews == 1){
  var red = block.getFieldValue('rr');  
  var blue = block.getFieldValue('gg');
  var green = block.getFieldValue('bb');
  }
...
Blockly.Arduino['ledi_ws2812x2led'] = function(block) {
  ...
  var numpixels = block.getFieldValue('led'); // количество пикселей в ленте
   ...
   var led1 = block.getFieldValue('led1');  
   var led2 = block.getFieldValue('led2');  
...
if(number == 6){
  var code = ' Pixcil = '+valueledon+'; \n NeoPix = '+valueledoff+' - Pixcil; \n  for (int count = 0; count < NeoPix; count++) { \n    Pixcil = Pixcil + 1; \n   strip'+numberstrip+'.setBrightness('+led1+'); \n  strip'+numberstrip1+'.setBrightness('+led2+'); \n   strip'+numberstrip+'.setPixelColor(Pixcil-1, strip'+numberstrip+'.Color('+red+','+green+','+blue+')); \n  strip'+numberstrip1+'.setPixelColor(Pixcil-1, strip'+numberstrip1+'.Color('+red1+','+green1+','+blue1+')); \n       } \n   Pixcil = '+valueledon+'; \n  strip'+numberstrip+'.show(); \n  strip'+numberstrip1+'.show(); \n  delay('+pixel_number+'); \n ';
  }
...
Blockly.Arduino['ledi_ws2812x4led'] = function(block) {
  ...
  var led2 = block.getFieldValue('led2');
    ...
    var numberstrip2 = block.getFieldValue('pin2'); // номер
  var pin_ledrgb2 = block.getFieldValue('pin2'); // пин ленты
  var led3 = block.getFieldValue('led3');
...
if(number == 6){
  var code = ' Pixcil = '+valueledon+'; \n NeoPix = '+valueledoff+' - Pixcil; \n  for (int count = 0; count < NeoPix; count++) { \n    Pixcil = Pixcil + 1; \n   strip'+numberstrip+'.setBrightness('+led1+'); \n  strip'+numberstrip1+'.setBrightness('+led2+'); \n strip'+numberstrip2+'.setBrightness('+led3+'); \n strip'+numberstrip3+'.setBrightness('+led4+'); \n  strip'+numberstrip+'.setPixelColor(Pixcil-1, strip'+numberstrip+'.Color('+red+','+green+','+blue+')); \n  strip'+numberstrip1+'.setPixelColor(Pixcil-1, strip'+numberstrip1+'.Color('+red1+','+green1+','+blue1+')); \n  strip'+numberstrip2+'.setPixelColor(Pixcil-1, strip'+numberstrip2+'.Color('+red2+','+green2+','+blue2+')); \n  strip'+numberstrip3+'.setPixelColor(Pixcil-1, strip'+numberstrip3+'.Color('+red3+','+green3+','+blue3+')); \n    } \n   Pixcil = '+valueledon+'; \n   strip'+numberstrip+'.show(); \n  strip'+numberstrip1+'.show(); \n   strip'+numberstrip2+'.show(); \n   strip'+numberstrip3+'.show(); \n   delay('+pixel_number+'); \n ';
  }
...
Blockly.Arduino["soft_write_ttl_uart_reset"]=function(block){
  ...
  Blockly.Arduino.includes_["soft_write_ttl_uart"+hexcode1+hexcode2+hexcode3+hexcode4]="   char x"+hexcode1+hexcode2+hexcode3+hexcode4+"m [6] ={0x5A,0xA5,0x05,0x82,0x"+hexcode1+""+hexcode2+",0x"+hexcode3+""+hexcode4+"}; \n int x"+hexcode1+hexcode2+hexcode3+hexcode4+"w = 0; \n  ";
    return '   x'+hexcode1+hexcode2+hexcode3+hexcode4+'w = '+content+'; \n '+
' nb =  x'+hexcode1+hexcode2+hexcode3+hexcode4+'w / 256; \n '+
' xb =  x'+hexcode1+hexcode2+hexcode3+hexcode4+'w % 256; \n '+
...
Blockly.Arduino['procedures_callreturn']=function(block){
    ...
    return code;//[code, Blockly.Arduino.ORDER_UNARY_POSTFIX];
};
...
Blockly.Arduino.definitions_['setups_sim800_init_ssc']='  void sms(String text, String phone)  \n '+     
 '{ \n '+   
...
Blockly.Arduino['sim800_sendc']=function(block){
    var valuetext_block = Blockly.Arduino.valueToCode(block, 'text_block', Blockly.Arduino.ORDER_ATOMIC);
    var valuetell_number = block.getFieldValue('tell_number');

    var code = ' sms(String('+valuetext_block+'), String("+'+valuetell_number+'"));   \n';
    return code
};
Blockly.Arduino['sim800_send_smsc']=function(block){
...
Blockly.Arduino['sim800_init_smsc']=function(block){
    var valuesms_statement = Blockly.Arduino.statementToCode(block, 'sms_statement');
    var valuesss_text = block.getFieldValue('sss_text');

    var code = '   if (inputString.indexOf("'+valuesss_text+'") > -1){ \n   '+valuesms_statement+' \n   } \n ';
   return code
};
...
Blockly.Arduino['motor_step_dir2'] = function(block) {

  var steeper_number = Blockly.Arduino.valueToCode(this, 'STEPPER_STEP_h', Blockly.Arduino.ORDER_ATOMIC);  
  var steeper_pin2 = Blockly.Arduino.valueToCode(this, 'PIN_STEEPER1', Blockly.Arduino.ORDER_ATOMIC);  
  var steeper_pin1 = Blockly.Arduino.valueToCode(this, 'PIN_STEEPER2', Blockly.Arduino.ORDER_ATOMIC);   
  var stepper_steprev = Blockly.Arduino.valueToCode(this, 'STEPPER_STEP_j', Blockly.Arduino.ORDER_ATOMIC);  
 
  Blockly.Arduino.variables_['define_stepper'] = ' #include <GyverStepper2.h> \n';
  Blockly.Arduino.definitions_['define_stepper'+steeper_pin1+steeper_pin2] = 'GStepper2<STEPPER2WIRE> stepper'+steeper_pin1+steeper_pin2+'(2048, '+steeper_pin1+', '+steeper_pin2+'); \n';
  Blockly.Arduino.setups_['define_stepper'+steeper_pin1+steeper_pin2] = ' stepper'+steeper_pin1+steeper_pin2+'.setTarget(0);  \n';

  var code = 'stepper'+steeper_pin1+steeper_pin2+'.setMaxSpeed('+steeper_number+');   \n   stepper'+steeper_pin1+steeper_pin2+'.setAcceleration('+steeper_number+' / 3); \n  stepper'+steeper_pin1+steeper_pin2+'.tick();   if (stepper'+steeper_pin1+steeper_pin2+'.ready()) { stepper'+steeper_pin1+steeper_pin2+'.setTarget('+stepper_steprev+');    }  \n';
  return code;
};

Blockly.Arduino['motor_step_dir3'] = function(block) {
  ...
  var code = ' stepper'+steeper_pin1+steeper_pin2+'.tick(); \n stepper'+steeper_pin1+steeper_pin2+'.setSpeed('+stepper_steprev+');   \n';
  return code;
};

 Blockly.Arduino["serial_read1x"]=function(block){
var color = this.getFieldValue('color'); 
    var code="Serial.readString()";
    return [code, Blockly.Arduino.ORDER_ATOMIC]
};
 Blockly.Arduino["serial_read2x"]=function(block){
var color = this.getFieldValue('color'); 
    var code="mySerial.readString()";
    return [code, Blockly.Arduino.ORDER_ATOMIC]
};
 Blockly.Arduino["serial_read3x"]=function(block){
var color = this.getFieldValue('color'); 
...
// Генерация JavaScript кода для блока

Blockly.Arduino['ardublock_block23']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
    var valuea1 = block.getFieldValue('a1');
	var valuea2 = block.getFieldValue('a2');
	var valuea3 = block.getFieldValue('a3');
    var valuea4 = block.getFieldValue('a4');
	var valuea5 = block.getFieldValue('a5');

	
 Blockly.Arduino.variables_['ardublock_block23'+valuea5] = ''+valuea1+' \n'+valuea2+' \n'+valuea3+' \n';
 Blockly.Arduino.setups_['ardublock_block23'+valuea5] = ' '+valuea4+' \n';
    var code = ''+value2+' \n ';
    return code
};
Blockly.Arduino['ardublock_block24']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
    var valuea1 = block.getFieldValue('a1');
	var valuea2 = block.getFieldValue('a2');
 ...
 Blockly.Arduino.variables_['ardublock_block24'+valuea5] = ''+valuea1+' \n'+valuea2+' \n'+valuea3+' \n';
 Blockly.Arduino.setups_['ardublock_block24'+valuea5] = ' '+valuea4+' \n';
    var code = value2;
    return [code, Blockly.Arduino.ORDER_ATOMIC] ;
};


Blockly.Arduino['ardublock_block25']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
    var value3 = Blockly.Arduino.valueToCode(block, 'block1', Blockly.Arduino.ORDER_ATOMIC);
    var value4 = block.getFieldValue('d4');
    var valuea1 = block.getFieldValue('a1');
	var valuea2 = block.getFieldValue('a2');
	var valuea3 = block.getFieldValue('a3');
    var valuea4 = block.getFieldValue('a4');
 ...
 Blockly.Arduino.variables_['ardublock_block25'+valuea5] = ''+valuea1+' \n'+valuea2+' \n'+valuea3+' \n';
 Blockly.Arduino.setups_['ardublock_block25'+valuea5] = ' '+valuea4+' \n';	
    var code = ''+value2+''+value3+''+value4+' \n';
    return code
};
Blockly.Arduino['ardublock_block26']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
    var value3 = Blockly.Arduino.valueToCode(block, 'block1', Blockly.Arduino.ORDER_ATOMIC);
    var value4 = block.getFieldValue('d4');
 ...
 Blockly.Arduino.variables_['ardublock_block26'+valuea5] = ''+valuea1+' \n'+valuea2+' \n'+valuea3+' \n';
 Blockly.Arduino.setups_['ardublock_block26'+valuea5] = ' '+valuea4+' \n';
    var code = ''+value2+''+value3+''+value4+'';
    return [code, Blockly.Arduino.ORDER_ATOMIC] ;
}; 
Blockly.Arduino['ardublock_block27']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
    var value3 = Blockly.Arduino.valueToCode(block, 'block1', Blockly.Arduino.ORDER_ATOMIC);
    var value4 = block.getFieldValue('d4');
	var value5 = Blockly.Arduino.valueToCode(block, 'block2', Blockly.Arduino.ORDER_ATOMIC);
    var value6 = block.getFieldValue('d5');
    var valuea1 = block.getFieldValue('a1');
	var valuea2 = block.getFieldValue('a2');
	var valuea3 = block.getFieldValue('a3');
    var valuea4 = block.getFieldValue('a4');
 ...
 Blockly.Arduino.variables_['ardublock_block27'+valuea5] = ''+valuea1+' \n'+valuea2+' \n'+valuea3+' \n';
 Blockly.Arduino.setups_['ardublock_block27'+valuea5] = ' '+valuea4+' \n';	
    var code = ''+value2+''+value3+''+value4+''+value5+''+value6+'\n';
    return code
};
Blockly.Arduino['ardublock_block28']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
    var value3 = Blockly.Arduino.valueToCode(block, 'block1', Blockly.Arduino.ORDER_ATOMIC);
    var value4 = block.getFieldValue('d4');
 ...
 Blockly.Arduino.variables_['ardublock_block28'+valuea5] = ''+valuea1+' \n'+valuea2+' \n'+valuea3+' \n';
 Blockly.Arduino.setups_['ardublock_block28'+valuea5] = ' '+valuea4+' \n';
    var code = ''+value2+''+value3+''+value4+''+value5+''+value6+'';
    return [code, Blockly.Arduino.ORDER_ATOMIC] ;
};
Blockly.Arduino['ardublock_block29']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
	var value3 = block.getFieldValue('d3');
 ...
 Blockly.Arduino.variables_['ardublock_block29'+valuea5] = ''+valuea1+' \n'+valuea2+' \n'+valuea3+' \n';
 Blockly.Arduino.setups_['ardublock_block29'+valuea5] = ' '+valuea4+' \n';
    var code = ''+value2+' \n '+value_statement+' \n '+value3+' \n';
    return code
};
Blockly.Arduino['ardublock_block30']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
    var value3 = Blockly.Arduino.valueToCode(block, 'block1', Blockly.Arduino.ORDER_ATOMIC);
    var value4 = block.getFieldValue('d4');
	var value_statement = Blockly.Arduino.statementToCode(block, 'statement');
	var value5 = block.getFieldValue('d5');
	var valuea1 = block.getFieldValue('a1');
	var valuea2 = block.getFieldValue('a2');
	var valuea3 = block.getFieldValue('a3');
    var valuea4 = block.getFieldValue('a4');
 ...
 Blockly.Arduino.variables_['ardublock_block30'+valuea5] = ''+valuea1+' \n'+valuea2+' \n'+valuea3+' \n';
 Blockly.Arduino.setups_['ardublock_block30'+valuea5] = ' '+valuea4+' \n';
    var code = ''+value2+''+value3+''+value4+' \n'+value_statement+' \n'+value5+' \n';
    return code
};
Blockly.Arduino['ardublock_block31']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
    var value3 = Blockly.Arduino.valueToCode(block, 'block1', Blockly.Arduino.ORDER_ATOMIC);
    var value4 = block.getFieldValue('d4');
	var value5 = Blockly.Arduino.valueToCode(block, 'block2', Blockly.Arduino.ORDER_ATOMIC);
    var value6 = block.getFieldValue('d5');
	var value7 = Blockly.Arduino.valueToCode(block, 'block3', Blockly.Arduino.ORDER_ATOMIC);
    var value8 = block.getFieldValue('d7');
	var valuea1 = block.getFieldValue('a1');
	var valuea2 = block.getFieldValue('a2');
	var valuea3 = block.getFieldValue('a3');
    var valuea4 = block.getFieldValue('a4');
 ...
 Blockly.Arduino.variables_['ardublock_block31'+valuea5] = ''+valuea1+' \n'+valuea2+' \n'+valuea3+' \n';
 Blockly.Arduino.setups_['ardublock_block31'+valuea5] = ' '+valuea4+' \n';	
    var code = ''+value2+''+value3+''+value4+''+value5+''+value6+''+value7+''+value8+'\n';
    return code
};
Blockly.Arduino['ardublock_block32']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
    var value3 = Blockly.Arduino.valueToCode(block, 'block1', Blockly.Arduino.ORDER_ATOMIC);
    var value4 = block.getFieldValue('d4');
	...
	var value7 = Blockly.Arduino.valueToCode(block, 'block3', Blockly.Arduino.ORDER_ATOMIC);
    var value8 = block.getFieldValue('d7');
	var value9 = Blockly.Arduino.valueToCode(block, 'block4', Blockly.Arduino.ORDER_ATOMIC);
    var value10 = block.getFieldValue('d10');
	var valuea1 = block.getFieldValue('a1');
	var valuea2 = block.getFieldValue('a2');
	var valuea3 = block.getFieldValue('a3');
    var valuea4 = block.getFieldValue('a4');
 ...
 Blockly.Arduino.variables_['ardublock_block32'+valuea5] = ''+valuea1+' \n'+valuea2+' \n'+valuea3+' \n';
 Blockly.Arduino.setups_['ardublock_block32'+valuea5] = ' '+valuea4+' \n';
    var code = ''+value2+''+value3+''+value4+''+value5+''+value6+''+value7+''+value8+''+value9+''+value10+'\n';
    return code
};
Blockly.Arduino['ardublock_block33']=function(block){
    var value1 = block.getFieldValue('d1');
    var value2 = block.getFieldValue('d2');
    var value3 = block.getFieldValue('d3');
	var value4 = block.getFieldValue('d4');
	var value5 = block.getFieldValue('d5');
	var value6 = block.getFieldValue('d6');
	
 Blockly.Arduino.variables_['ardublock_block33'+value6] = ''+value2+' \n'+value3+''+value4+''+value5+' \n';
...
Blockly.Arduino['text_st']=function(block){
    var valuestring_int_block = Blockly.Arduino.valueToCode(block, 'string-int-block', Blockly.Arduino.ORDER_ATOMIC);
    var code = valuestring_int_block + '.c_str()';
    return [code, Blockly.Arduino.ORDER_ATOMIC];
};


Blockly.Arduino['ESPWEB_UDP1']=function(block){
    var value_block = Blockly.Arduino.valueToCode(block, '_block', Blockly.Arduino.ORDER_ATOMIC);
    Blockly.Arduino.includes_['ESPWEB_UDP1'] = ' #if defined(ESP8266)  \n #include <ESP8266WiFi.h>  \n #endif  \n #if defined(ESP32)  \n #include "WiFi.h"  \n #endif  \n #include <WiFiUdp.h>  \n ';
    Blockly.Arduino.variables_['ESPWEB_UDP1'] = 'int valueToSend = 0;  \n ';
    Blockly.Arduino.userFunctions_['ESPWEB_UDP1'] = 'WiFiUDP udp; \n ';
	Blockly.Arduino.setups_['ESPWEB_UDP1']=' Serial.begin(9600); \n WiFi.softAP("ESP8266_AP", "12345678"); \n delay(1000); \n Serial.print("Access Point IP address: "); \n Serial.println(WiFi.softAPIP()); \n udp.begin(1234);  \n  ';
  
    var code = 'IPAddress receiverIp(192, 168, 4, 2); \n  udp.beginPacket(receiverIp, 1234); \n  valueToSend = '+value_block+'; \n  udp.write((uint8_t*)&valueToSend, sizeof(valueToSend)); \n  udp.endPacket();  \n ';
    return code
};
Blockly.Arduino['ESPWEB_UDP2']=function(block){
    var value_block = Blockly.Arduino.valueToCode(block, '_block', Blockly.Arduino.ORDER_ATOMIC);
    Blockly.Arduino.includes_['ESPWEB_UDP2'] = ' #if defined(ESP8266)  \n #include <ESP8266WiFi.h>  \n #endif  \n #if defined(ESP32)  \n #include "WiFi.h"  \n #endif  \n #include <WiFiUdp.h>  \n ';
    Blockly.Arduino.variables_['ESPWEB_UDP2'] = 'int receivedValue = 0;  \n ';
    Blockly.Arduino.userFunctions_['ESPWEB_UDP2'] = 'WiFiUDP udp; \n ';
	Blockly.Arduino.setups_['ESPWEB_UDP2']='  Serial.begin(9600);  \n  WiFi.begin("ESP8266_AP", "12345678");  \n   while (WiFi.status() != WL_CONNECTED) {  \n    delay(500);  \n   Serial.println("Connecting to WiFi...");  \n    }  \n    Serial.print("Connected to WiFi with IP: ");  \n   Serial.println(WiFi.localIP());  \n   udp.begin(1234);  \n  ';
  
    var code = '  if (udp.parsePacket()) {  \n  udp.read((uint8_t*)&receivedValue, sizeof(receivedValue));  \n   '+value_block+' = receivedValue;  \n }  \n ';
    return code
};
...
Blockly.Arduino["soft_init_ttlesp8266"]=function(block){
    var dropdown_pin1=Blockly.Arduino.valueToCode(block, 'PIN1', Blockly.Arduino.ORDER_ATOMIC);
    var dropdown_pin2=Blockly.Arduino.valueToCode(block, 'PIN2', Blockly.Arduino.ORDER_ATOMIC);
    var dropdown_speed=block.getFieldValue("SPEED");
    Blockly.Arduino.includes_["define_ss"]="#include <SoftwareSerial.h>";
  Blockly.Arduino.definitions_["define_ss"]="SoftwareSerial mySerial(" + dropdown_pin1 + "," + dropdown_pin2 + ");";
    Blockly.Arduino.setups_["setup_sserial"]="mySerial.begin(" + dropdown_speed + ");";
...
Blockly.Arduino["soft_init_ttlesp32"]=function(block){
    var dropdown_pin1=Blockly.Arduino.valueToCode(block, 'PIN1', Blockly.Arduino.ORDER_ATOMIC);
    var dropdown_pin2=Blockly.Arduino.valueToCode(block, 'PIN2', Blockly.Arduino.ORDER_ATOMIC);
    var dropdown_speed=block.getFieldValue("SPEED");
    Blockly.Arduino.includes_["define_ss"]="HardwareSerial mySerial(1);";
    Blockly.Arduino.setups_["setup_sserial"]="mySerial.begin(" + dropdown_speed + ", SERIAL_8N1, " + dropdown_pin1 + ", " + dropdown_pin2 + "); ";
    return ""
};

Blockly.Arduino["soft_init2esp32"]=function(block){
    var dropdown_pin1=Blockly.Arduino.valueToCode(block, 'PIN1', Blockly.Arduino.ORDER_ATOMIC);
    var dropdown_pin2=Blockly.Arduino.valueToCode(block, 'PIN2', Blockly.Arduino.ORDER_ATOMIC);
    var dropdown_speed=block.getFieldValue("SPEED");
    Blockly.Arduino.includes_["define_ss"]="#include \"driver/uart.h\"  \n #define UART_NUM UART_NUM_2  \n  #define BUF_SIZE (1024)  \n HardwareSerial mySerial(2);";  
    Blockly.Arduino.setups_["setup_sserial"]="mySerial.begin(" + dropdown_speed + ", SERIAL_8N1, " + dropdown_pin1 + ", " + dropdown_pin2 + "); ";
    return ""
};
...
Blockly.Arduino["soft_write_ttl_uartesp32"]=function(block){
    var content=Blockly.Arduino.valueToCode(block, "CONTENT", Blockly.Arduino.ORDER_ATOMIC);
  var hexcode1 = block.getFieldValue("TEXT1");
  var hexcode2 = block.getFieldValue("TEXT2");
  var hexcode3 = block.getFieldValue("TEXT3");
  var hexcode4 = block.getFieldValue("TEXT4");
  Blockly.Arduino.includes_["soft_write_ttl_uart"+hexcode1+hexcode2+hexcode3+hexcode4]="   byte x"+hexcode1+hexcode2+hexcode3+hexcode4+"m [7] ={0x5A,0xA5,0x05,0x82,0x"+hexcode1+""+hexcode2+",0x"+hexcode3+""+hexcode4+",0x00}; \n int x"+hexcode1+hexcode2+hexcode3+hexcode4+"w = 0; \n  ";
    return "   x"+hexcode1+hexcode2+hexcode3+hexcode4+"w = " + content + "; \n    mySerial.write(x"+hexcode1+hexcode2+hexcode3+hexcode4+"m , 7); \n    mySerial.write(x"+hexcode1+hexcode2+hexcode3+hexcode4+"w); \n  "
};

  Blockly.Arduino["soft_write_ttl_uart2esp32"]=function(block){
    var content=Blockly.Arduino.valueToCode(block, "CONTENT", Blockly.Arduino.ORDER_ATOMIC);
  var hexcode1 = block.getFieldValue("TEXT1");
  var hexcode2 = block.getFieldValue("TEXT2");
  var hexcode3 = block.getFieldValue("TEXT3");
  var hexcode4 = block.getFieldValue("TEXT4");
  Blockly.Arduino.includes_["soft_write_ttl_uart2"]=" int nb = 0; \n  int xb = 0; \n ";
  Blockly.Arduino.includes_["soft_write_ttl_uart"+hexcode1+hexcode2+hexcode3+hexcode4]="   byte x"+hexcode1+hexcode2+hexcode3+hexcode4+"m [6] ={0x5A,0xA5,0x05,0x82,0x"+hexcode1+""+hexcode2+",0x"+hexcode3+""+hexcode4+"}; \n int x"+hexcode1+hexcode2+hexcode3+hexcode4+"w = 0; \n  ";
    return '   x'+hexcode1+hexcode2+hexcode3+hexcode4+'w = '+content+'; \n '+
' nb =  x'+hexcode1+hexcode2+hexcode3+hexcode4+'w / 256; \n '+
' xb =  x'+hexcode1+hexcode2+hexcode3+hexcode4+'w % 256; \n '+
' mySerial.write(x'+hexcode1+hexcode2+hexcode3+hexcode4+'m , 6); \n '+
...
Blockly.Arduino["soft_write_ttl_uart_gesp32"]=function(block){
    var content=Blockly.Arduino.valueToCode(block, "CONTENT", Blockly.Arduino.ORDER_ATOMIC);
  var hexcode1 = block.getFieldValue("TEXT1");
  Blockly.Arduino.includes_["soft_write_ttl_uart"+hexcode1]="   byte x"+hexcode1+"m [13] ={0x5A, 0xA5, 0x0B, 0x82, 0x03, 0x10, 0x5A, 0xA5, 0x01, 0x00, 0x0"+hexcode1+", 0x01, 0x00}; \n int x"+hexcode1+"w = 0; \n  ";
    return "   x"+hexcode1+"w = " + content + "; \n    mySerial.write(x"+hexcode1+"m , 13); \n    mySerial.write(x"+hexcode1+"w); \n  "
};


Blockly.Arduino["soft_write_ttl_uart_ledesp32"]=function(block){
    var content=Blockly.Arduino.valueToCode(block, "CONTENT", Blockly.Arduino.ORDER_ATOMIC);
  Blockly.Arduino.includes_["includes_soft_write_ttl_uart_led"]=" byte leddwin [10] ={0x5A,0xA5,0x07,0x82,0x00,0x82,0x00,0x00,0x00,0x00}; \n ";
     var code = "  leddwin[6]=" + content + "; \n    leddwin[7]=" + content + "; \n     mySerial.write(leddwin,10); \n  ";
   return code
};

Blockly.Arduino["soft_write_ttl_uart_resetesp32"]=function(block){
    var content=Blockly.Arduino.valueToCode(block, "CONTENT", Blockly.Arduino.ORDER_ATOMIC);
  Blockly.Arduino.includes_["includes_soft_write_ttl_uart_reset"]=" byte resetdwin [10] ={0x5A,0xA5,0x07,0x82,0x00,0x04,0x55,0xAA,0x5A,0xA5}; \n ";
     var code = "   mySerial.write(resetdwin,10); \n  ";
   return code
};
